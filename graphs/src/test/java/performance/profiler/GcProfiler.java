...
public class GcProfiler implements InternalProfiler {
  ...
  private final List<Long> usedAfterGc = Collections.synchronizedList(new ArrayList<>());
  private final List<Long> committedAfterGc = Collections.synchronizedList(new ArrayList<>());
  ...
  static class HotspotAllocationSnapshot {
    ...
    /**
     * Takes a snapshot of thread allocation counters. The method might allocate, however it is assumed that allocations
     * made by "current thread" will be excluded from the result while performing {@link
     * HotspotAllocationSnapshot#subtract(HotspotAllocationSnapshot)}
     *
    ...
    public static HotspotAllocationSnapshot create() {
      ...
      try {
        ...
        long[] allocatedBytes = (long[]) getBytes.invoke(threadMXBean, threadIds);
    ...
    /**
     * Estimates allocated bytes based on two snapshots. The problem is threads can come and go while performing the
     * benchmark, thus we would miss allocations made in a thread that was created and died between the snapshots.
     * <p/>
